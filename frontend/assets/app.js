...
  function summaryChip({label, owned}){
    const el = h('div',{class:`tile tile--summary ${owned ? 'tile--owned' : 'tile--selected'}`}, label);
    if (owned) {
      el.appendChild(h('span',{class:'pill pill--owned'},'Posiadany'));
    } else {
      el.appendChild(h('span',{class:'pill pill--selected'},'Wybrany'));
    }
    return el;
  }
...
        compensationDiscount = computeCompensation(); // wartość do odjęcia
      }
...
      // Widok:
      if (state.global.packageMode) {
        // Pakiet ON → Prezentacja w formie "chips"
        const chipsContainer = h('div', { class: 'tiles' }); // Reuse 'tiles' class for flex container

        // Posiadane produkty główne
        ownedMainLabels.forEach(lab => {
          chipsContainer.append(summaryChip({ label: lab, owned: true }));
        });
        
        // Nowe (wybrane) produkty główne
        selectedMainLabels.forEach(lab => {
          chipsContainer.append(summaryChip({ label: lab, owned: false }));
        });
        
        // Wybrane usługi
...
        // Dodatkowi użytkownicy
        if (extraQty > 0) {
          const extraUserLabel = `Dodatkowi użytkownicy (${extraQty})`;
          chipsContainer.append(summaryChip({ label: extraUserLabel, owned: false }));
        }
        
        summaryBox.append(chipsContainer);
      } else {
